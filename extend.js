							...
							me.store.each(function(rec){
								if (rec.data['flag'] == 1) {
									if (show && rec.data['type'] == 'phone call' || rec.data['type'] == 'appointment') {
											var ico = 'a-call';
											if (rec.data['type'] == 'appointment')
												ico = 'a-event';
											if (!reuse) {																
												reuse = Ext.create('widget.uxNotification', {
													position: 'tl',
													cls: 'ux-notification-light',
													closable: false,										
													title: '',
													html: '<img src="images/'+ico+'.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=workspace"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> remind by '+rec.data['owner']
												}).show();
											} else {
												reuse.update('<img src="images/'+ico+'.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=workspace"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> remind by '+rec.data['owner']);
												reuse.show();
											}
									} else
										...
										if (rec.data['status'] == 'assigned' && show) {											
											if (!reuse) {				
												reuse = Ext.create('widget.uxNotification', {
													...
													closable: false,										
													title: '',
													html: '<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> assigned by '+rec.data['owner']
												}).show();
											} else {
												reuse.update('<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> assigned by '+rec.data['owner']);
												reuse.show();
											}
										} else
										if (rec.data['status'] == 'remind' && show) {								
											if (!reuse) {				
												reuse = Ext.create('widget.uxNotification', {
													...
													closable: false,										
													title: '',
													html: '<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> remind by '+rec.data['owner']
												}).show();
											} else {
												reuse.update('<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> remind by '+rec.data['owner']);
												reuse.show();
											}
										} else
										if (rec.data['status'] == 'commented' && show) {					
											if (!reuse) {				
												reuse = Ext.create('widget.uxNotification', {
													...
													closable: false,										
													title: '',
													html: '<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> posted by '+rec.data['owner']
												}).show();
											} else {
												reuse.update('<img src="images/deal.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=deals"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> posted by '+rec.data['owner']);
												reuse.show();
											}
										} else
											deal++;
									...
									{									
										if (!reuse) {				
											reuse = Ext.create('widget.uxNotification', {
												...
												closable: false,										
												title: '',
												html: '<img src="images/a-task.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=workspace"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> assigned by '+rec.data['owner']
											}).show();
										} else {
											reuse.update('<img src="images/a-task.png" width="16px" style="float:left; margin-right: 5px;"></img><a href="?pk=workspace"><b>'+rec.data['subject']+' ('+rec.data['type']+')</b></a></br> assigned by '+rec.data['owner']);
											reuse.show();
										}
	...
	callBack: function(store) {
		...
		if (me.func == 'crm_customer_activity_list' && pk == 'deals') {
			...
			store.each(function(rec){
				if (i == 0 && rec.get('status') == 'pending' && rec.get('remind_at') != null) {				
	...
	getEditor: function(name, value) {
		...
		if (name == 'status' && me.modelName == 'CRM_RISK_QUESTION')
		{
			return {
			  ...
			  store: Ext.create('Ext.data.Store', {
  				  model: 'CRM_PREV',
 				  data: [{value: '0', name: 'Сөрөг'},{value: '1', name: 'Эерэг'}]
              }),
			  name: name,
			  queryMode: 'local',
		      displayField: 'name',
